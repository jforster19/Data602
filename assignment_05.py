...
def exercise01():
    ...
    df = pd.DataFrame(np.random.randint(1,8,size=(3,4)))
...
def exercise11(n):
    ...
    array_reshaped = np.reshape(array_1d,newshape=(3,round(n/3)))
    # ------ Place code above here /\ /\ /\ ------  
...
def exercise12(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    checkerboard_matrix = np.tile(np.array([1,0]),(2*n,2*n))
    # ------ Place code above here /\ /\ /\ ------ 
...
def exercise14(words):
    ...
    df = pd.DataFrame([len(word) for word in words],columns=['words'],index=[words])
    # ------ Place code above here /\ /\ /\ ------ 
    return df
